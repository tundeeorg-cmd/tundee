...
import type { ChecklistStep, Language } from '@/lib/types';
...
export default function ChecklistUI({ steps, lang }: Props) {
        ...
        {displaySteps.map((step) => {
          ...
          return (
            <li key={step.id}>
              ...
              >
                <div
                  className={`flex items-center gap-4 p-4 rounded-[12px] border transition-all duration-200 ${
                    isOpen
                      ? 'border-[#F0A500] bg-[#FFF8E7]'
                      : 'border-[#E5E5EA] bg-white hover:border-[#F0A500]/50'
                  }`}
                >
                  {/* Step number */}
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold shrink-0 ${
                      isOpen ? 'bg-[#F0A500] text-white' : 'bg-[#F5F5F7] text-[#6E6E73]'
                    }`}
                  >
                    {step.step_number}
                  </div>
                  <span
                    className={`font-medium flex-1 ${isOpen ? 'text-[#1D1D1F]' : 'text-[#1D1D1F]'}`}
                    style={{ fontFamily: lang === 'th' ? 'Sarabun, sans-serif' : 'DM Sans, sans-serif' }}
                  >
                    {name}
                  </span>
                  <svg
                    className={`w-4 h-4 text-[#6E6E73] transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
              ...
              {isOpen && desc && (
                <div className="px-4 pt-2 pb-4 mx-0">
                  <p
                    className="text-sm text-[#6E6E73] leading-relaxed pl-12"
                    style={{ fontFamily: lang === 'th' ? 'Sarabun, sans-serif' : 'DM Sans, sans-serif' }}
                  >
                    {desc}
                  </p>
