...
const HomeIcon = ({ active }: { active: boolean }) => (
  <svg className={`w-6 h-6 ${active ? 'text-[#2E6BE6]' : 'text-[#ADADB8]'}`} fill={active ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
...
const SearchIcon = ({ active }: { active: boolean }) => (
  <svg className={`w-6 h-6 ${active ? 'text-[#2E6BE6]' : 'text-[#ADADB8]'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function BottomNav() {
  ...
  return (
    <nav
      className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-[#161B27] border-t border-[#E5E5EA] dark:border-[#232B3E]"
          ...
          >
            ...
            <span
              className={`text-[10px] font-medium leading-none ${
                item.active ? 'text-[#2E6BE6]' : 'text-[#ADADB8] dark:text-[#636366]'
              }`}
