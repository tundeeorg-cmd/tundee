...
const HomeIcon = ({ active }: { active: boolean }) => (
  <svg className={`w-6 h-6 ${active ? 'text-[#F0A500]' : 'text-[#ADADB8]'}`} fill={active ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
...
const SearchIcon = ({ active }: { active: boolean }) => (
  <svg className={`w-6 h-6 ${active ? 'text-[#F0A500]' : 'text-[#ADADB8]'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function BottomNav() {
  ...
  return (
    <nav
      className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-[#1C1C1E] border-t border-[#E5E5EA] dark:border-[#38383A]"
          ...
          >
            ...
            <span
              className={`text-[10px] font-medium leading-none ${
                item.active ? 'text-[#F0A500]' : 'text-[#ADADB8] dark:text-[#636366]'
              }`}
